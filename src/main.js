 ...
 * ============================================================ */
$(() => { errorCatched(initFarm)(); });
$(window).on('pagehide', () => { try { window.parent['__STAR_TAVERN_FARM__']?.destroy?.(); } catch (e) {} });
...
function initFarm() {
  ...
  function loadState() {
    ...
    if (!S.pets) S.pets = ['slime', 'octo'];            // Chuyển đổi giai đoạn 3: người chơi cũ được biên chế chính thức hai bé ở bờ ruộng
    if (!S.petsOut) S.petsOut = S.pets.slice(0, 6);     // v0.6b: đội ra sân (giới hạn 6)
    if (!S.jobCfg) S.jobCfg = {};                        // Cấu hình ghi nhớ cho gieo hạt / bón phân
    if (!S.petFind) S.petFind = {};                      // Bộ đếm giờ tìm kho báu
    if (!S.theme) S.theme = 'sakura';                  // v1.0: chủ đề giao diện ưa thích (toàn cục)
    if (!S.page) S.page = 1;                             // Chuyển đổi v0.8: nông trại ba trang
    /* Chuyển đổi v0.9b (#49): dâu tây bí ẩn cũ (mysb*) trong túi/đang trồng được gộp vào dâu tây chính thức (moonberry) */
    ...
    if (!S.witch) S.witch = { nextAt: now(), leaveAt: 0, missed: 0, order: null };   // v0.8b: phù thuỷ tròn (lần ghé đầu = lần mở bảng đầu tiên sau khi nhập dữ liệu)
    if (!S.shards) S.shards = { prism: 0, star: 0 };     // v1.0: mảnh vỡ (lăng quang = đổi đơn / ngôi sao = triệu hồi phù thuỷ)
    ...
    [S.plots, S.plots2, S.plots3].forEach(arr => arr.forEach(p => {   // Chuyển đổi save cũ: số vụ / dấu bón phân
  ...
  function save(immediate) {
    ...
    const doSave = () => updateVariablesWith(v => { v[NS] = S; return v; }, { type: 'global' });
    if (immediate) doSave(); else saveTimer = setTimeout(doSave, 500);
    try { updateInjection(); } catch (e) {}               // Công tắc ②: tình hình vườn rau cập nhật theo save
  }
